...
class SafetyAdvisorApp:
    ...
    def _run_file_scan(self, filepath: str):
        ...
        def worker():
            try:
                ...
                self.root.after(0, lambda: self._display_result(result, self.file_result_frame))
                ...
                self.root.after(0, lambda: self._show_message(
                    ...
                    f"Something went wrong while checking this file.\n\n"
                    f"Error: {exc}\n\nPlease try again.",
                    RISK_CAUTION,
                ))
                self.root.after(0, lambda: self._set_status("Check failed — see details above"))
    ...
    def _on_check_url(self):
        ...
        def worker():
            try:
                ...
                self.config.add_scan_history(result)
                self.last_scan_result = result
                self.root.after(0, lambda: self._display_result(result, self.url_result_frame))
                self.root.after(0, lambda: self._set_status("Ready"))
            except Exception as exc:
                log.exception("URL scan failed for %s", url)
                self.root.after(0, lambda: self._show_message(
                    self.url_result_frame,
                    f"Something went wrong while checking this website.\n\n"
                    f"Error: {exc}\n\nPlease try again.",
                    RISK_CAUTION,
                ))
                self.root.after(0, lambda: self._set_status("Check failed — see details above"))
    ...
    def _clear_history(self):
        ...
        if not confirmed:
            return
        self.config._data["scan_history"] = []
        self.config.save()
        self._refresh_history()
    ...
    def _build_settings_page(self) -> tk.Frame:
        ...
        contact_card.pack(fill="x", pady=(10, 4))

        # --- Downloads Folder ---
        dl_card = self._card(page)
        tk.Label(
            dl_card, text="📂  Downloads Folder to Watch",
            font=self.font_heading, fg=COLOURS["accent"], bg=COLOURS["panel"], anchor="w"
        ).pack(fill="x", pady=(0, 4))
        tk.Label(
            dl_card,
    ...
    def on_new_download_detected(self, filepath: str):
        ...
        self.root.after(0, lambda: self._auto_scan_popup(filepath))
